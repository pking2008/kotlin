...
class JvmBuiltinCallLowering(val context: JvmBackendContext) : FileLoweringPass {
    ...
    private fun isPrimitiveTypeConstantOrLocalLoad(expression: IrExpression): Boolean {
    ...
    private fun isNullCheckOfPrimitiveTypeValue(call: IrCall, context: JvmBackendContext): Boolean {
        if (call.symbol == context.irBuiltIns.eqeqSymbol) {
            ...
            // be null.
            return (right.isNullConst() && isPrimitiveTypeConstantOrLocalLoad(left))
                    || (left.isNullConst() && isPrimitiveTypeConstantOrLocalLoad(right))
        }
        return false
    }
